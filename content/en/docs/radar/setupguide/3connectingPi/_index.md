...
If the wifi static IP address bugs out and you can't ssh into the Pi OR the Pi stops connecting to the internet (can't ping google.com or 8.8.8.8), try restarting the Pi. Also try setting the ipv4.method back to auto, restart it, and back to manual. Do this by running the following commands.
- `sudo nmcli connection modify "wifi connection" ipv4.method auto`
- `sudo systemctl restart NetworkManager`
- `sudo reboot`
- `sudo nmcli connection modify "wifi connection" ipv4.method manual` 
- `sudo systemctl restart NetworkManager`
- `sudo reboot`
I haven't figured out what is causing this so if you do figure it out feel free to add to the documentation!
{{% /alert %}}
